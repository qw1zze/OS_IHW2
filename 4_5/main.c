...
int main() {
    ...
    char shar[] = "/shared-memory";

    int shm = shm_open(shar, O_CREAT | O_RDWR, 0666);
    ftruncate(shm, sizeof(Forks));
    Forks *forks = mmap(NULL, sizeof(Forks), PROT_READ | PROT_WRITE, MAP_SHARED, shm, 0);
    if (forks == MAP_FAILED) {
        perror("failure nmap");
        exit(EXIT_FAILURE);
    }
